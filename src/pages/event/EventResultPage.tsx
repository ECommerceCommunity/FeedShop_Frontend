...
const EventResultPage: React.FC = () => {
  ...
  const handleCreateResult = async (eventId: number) => {
    try {
      ...
      const errorMessage = err.response?.data?.message || "이벤트 결과 생성에 실패했습니다.";
      
      // 참여자가 없는 경우 더 명확한 안내
      if (errorMessage.includes("참여자가 없습니다")) {
        alert("이벤트에 참여자가 없어서 빈 결과로 생성됩니다.\n\n나중에 참여자가 추가되면 재계산할 수 있습니다.");
      } else {
        alert(errorMessage);
      }
    } finally {
  ...
  const handleProcessRewards = async (eventId: number) => {
    try {
      ...
      // 참여자가 없는 경우에도 성공으로 처리
      if (err.response?.status === 400 && err.response?.data?.message?.includes("참여자가 없습니다")) {
        const emptyResult = {
          eventId: eventId,
          totalParticipants: 0,
          successfulRewards: 0,
          failedRewards: 0,
          rewardDetails: [],
          message: "참여자가 없어서 보상 지급 대상이 없습니다. 결과가 정상적으로 처리되었습니다."
        };
        setRewardResult(emptyResult);
        setShowRewardModal(true);
        
        // 참여자가 없는 경우에도 결과 목록 업데이트
        const updatedResult = await EventService.getEventResult(eventId);
        setEventResults(prev => prev.map(r => r.eventId === eventId ? updatedResult : r));
      } else {
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {!loading && !error && (
          ...
          <button
                                ...
                                disabled={processingReward || result?.resultDetails.some(d => d.rewardProcessed)}
                                className={`px-6 py-3 rounded-xl transition-colors ${
                                  result?.resultDetails.some(d => d.rewardProcessed)
                                    ? 'bg-green-600 text-white cursor-not-allowed'
                                    : 'bg-purple-600 text-white hover:bg-purple-700 disabled:opacity-50'
                                }`}
                              >
                                {processingReward ? '처리중...' : 
                                 result?.resultDetails.some(d => d.rewardProcessed) ? '✅ 완료됨' : '보상 지급'}
          </button>
                            </>
                          )}
                        </div>
                          ...
                          {result.resultDetails.some(d => d.rewardProcessed) && (
                            <div className="mt-3 p-3 bg-green-50 border border-green-200 rounded-lg">
                              <div className="flex items-center gap-2">
                                ...
                                <span className="text-green-800 font-semibold">보상 지급이 완료되었습니다</span>
                              </div>
                              <p className="text-green-700 text-sm mt-1">
                                모든 참여자에게 포인트, 뱃지점수, 쿠폰이 정상적으로 지급되었습니다.
                              </p>
                            </div>
                          )}
                        </div>
                ...
                onClick={() => {
                  ...
                  if (rewardResult) {
                    const refreshResults = async () => {
                      try {
                        const updatedResult = await EventService.getEventResult(rewardResult.eventId);
                        setEventResults(prev => prev.map(r => r.eventId === rewardResult.eventId ? updatedResult : r));
                      } catch (err) {
                        console.error("결과 새로고침 실패:", err);
                      }
                    };
                    refreshResults();
                  }
