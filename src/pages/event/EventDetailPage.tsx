  ...
  return (
    <div className="p-5 max-w-3xl mx-auto">
      ...
      <div className="relative h-56 mb-6">
        ...
        <span className={`absolute top-4 left-4 px-3 py-1 rounded-full text-white text-sm ${event.status === 'upcoming' ? 'bg-blue-400' : event.status === 'ongoing' ? 'bg-green-500' : 'bg-gray-400'}`}>{event.status === 'upcoming' ? '진행 예정' : event.status === 'ongoing' ? '진행중' : '종료'}</span>
      </div>
      <div className="mb-6">
        <div className="flex items-center gap-3 mb-2">
          <h1 className="text-3xl font-bold text-gray-900">{event.title}</h1>
          <span className="bg-[#87CEEB] bg-opacity-10 text-[#87CEEB] px-3 py-1 rounded-full text-sm font-medium">
            {event.type === 'battle' ? '배틀' : event.type === 'mission' ? '미션' : '다수'}
          </span>
        </div>
          ...
          {event.rewards.map((reward) => (
            <div key={reward.rank} className="bg-gray-50 p-6 rounded-lg">
              <div className="flex items-center mb-3">
                <div className="bg-[#87CEEB] text-white w-8 h-8 rounded-full flex items-center justify-center font-bold mr-3">{reward.rank}</div>
