...
import { CreateOrderRequest, OrderDetail, OrderListResponse, OrderResponse, PurchasedProduct } from "types/order";
...
export class OrderService {
    ...
    static async getOrders(
        page: number = 0,
        size: number = 10,
        status?: "ORDERED" | "SHIPPED" | "DELIVERED" | "CANCELED" | "RETURNED" | "ALL"
    ...
    static async getPurchasedProducts(): Promise<PurchasedProduct[]> {
        try {
            const response = await axiosInstance.get<ApiResponse<{ items: PurchasedProduct[] }>>(
    ...
    static async updateOrderStatus(
        ...
        try {
            const response = await axiosInstance.post <ApiResponse<{ orderId: number; status: string }>>(
