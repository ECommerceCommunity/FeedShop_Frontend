...
class EventService {
  ...
  async getFeedAvailableEvents(): Promise<FeedEventDto[]> {
    try {
      const response = await axiosInstance.get<ApiResponse<FeedEventDto[]>>('/api/v1/events/feed-available');
  ...
  async createEvent(eventData: EventCreateRequestDto): Promise<EventDto> {
    try {
      const response = await axiosInstance.post('/api/v1/events', eventData);
  ...
  async updateEvent(eventId: number, eventData: EventUpdateRequestDto): Promise<EventDto> {
    try {
      const response = await axiosInstance.put(`/api/v1/events/${eventId}`, eventData);
      return response.data.data;
    } catch (error) {
