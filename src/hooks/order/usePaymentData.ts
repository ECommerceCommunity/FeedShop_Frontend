...
export const usePaymentData = (): UsePaymentDataReturn => {
  ...
  useEffect(() => {
    ...
    if (locationState.isDirectOrder) {
      ...
      if (
        locationState.directOrderItems &&
        locationState.directOrderItems.length > 0
      ) {
        const directOrderItems = locationState.directOrderItems.map(
          (item: any) => ({
  ...
  const applyCouponByCode = async (couponCode: string) => {
    try {
      ...
      const appliedCoupon = await couponService.useCoupon({
        ...
        orderAmount: state.items.reduce(
      ...
      setState((prev) => ({
        ...
        availableCoupons: prev.availableCoupons.some(
          (c) => c.couponName === appliedCoupon.couponName
        )
